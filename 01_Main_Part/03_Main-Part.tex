...
\subsubsection{Methode mit einem Parameter}
...
Da der Typ des Parameters ein Typ der Hostsprache sein kann, wird dadurch Literale und Ausdrücke der Hostsprache in die DSL selbst eingebettet werden. Da der Typ auch ein eigener sein kann, kann eine Rekursion erreicht werden, indem der mit dem Parameter (an den Prototypen) spezifizierte Typ verwendet wird. Somit wird die Mächtigkeit einer kontextfreien Sprache erreicht.
...
\chapter{Testlauf mit anschaulichen Beispielen}\label{SEC:Examples}
...
\subsection{Graph}
Der Graph in Abbildung~\ref{FIG:ExprGraph} zeigt den aus der EBNF-Grammatik generierten Graphen. Wie zuvor beschrieben wurden hierfür alle Nicht-Terminale durch ihre entsprechenden Regeln ersetzt, wodurch nur ein zusammenhängender Graph entsteht; die Regel \verb|<expr>| diente hierbei als Einstiegs-Regel.

Abbildungen~\ref{FIG:ExprSubGraphsOne}~und~\ref{FIG:ExprSubGraphsTwo} zeigen einige Graphen, die aus den Regeln erstellt werden, bevor sie zu dem Gesamtgraphen zusammengesetzt werden. Abbildung~\ref{FIG:ExprStartRule} zeigt die obligatorische Startregel, dessen Ziel-Knoten (Scope1) sich als End-Knoten des gesamten Graphen in Abbildung~\ref{FIG:ExprGraph} wiederfindet. In Abbildung~\ref{FIG:ExprRule} werden die Operatoren (inkl. Alternative für verschiedene Datentypen) eingesetzt. Die Rechenoperationen sind analog wie Abbildung~\ref{FIG:ExprAlternativeTypes} aufgebaut mit einem alternativen Strang für den Typ Double und Expr. Da in der Schleife viele Alternativen vorkommen ergeben sich die Abbildung~\ref{FIG:ExprGraph} vielen alternativen Schleifen.

\begin{figure}[ht]
\centering
  \begin{subfigure}[b]{0.3\textwidth}
  	\includegraphics[width=\textwidth]{Expr_Rule_00}
  	\caption{Start-Regel (Expr)}
  	\label{FIG:ExprStartRule}
  \end{subfigure}
  \begin{subfigure}[b]{0.3\textwidth}
  	\includegraphics[width=\textwidth]{Expr_Rule_01}
  	\caption{Regel eines Ausdrucks (startExpr)}
  	\label{FIG:ExprRule}
  \end{subfigure}
  \begin{subfigure}[b]{0.3\textwidth}
  	\includegraphics[width=\textwidth]{Expr_Rule_02}
  	\caption{Alternative für Datentypen (lhs)}
  	\label{FIG:ExprAlternativeTypes}
  \end{subfigure}
  \caption{Regeln für den Einstieg, Ausdruck und Alternative für Datentypen}
  \label{FIG:ExprSubGraphsOne}
\end{figure}

\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{Expr_Rule_03}
	\caption{Regel der Alternative für verschiedene Rechenarten (operatorRhs)}
	\label{FIG:ExprSubGraphsTwo}
\end{figure}
...
Das BeginInterface (Listing~\ref{LST:JOIBeginScopeJava}) zeigt, dass die Konsistenzregeln für den gesamten Graph gelten, denn die Regel \verb|componentName| (Listing~\ref{LST:JOIBNF}, Zeilen 10-12) beinhaltet nur den Typen für jeweils die Komponente oder den Singleton (vgl. Zeile 5). Abbildung~\ref{LST:JOIBeginScopeScala} zeigt, dass statt einem Java-Interface auch ein Trait aus der Sprache Scala generiert werden kann.
\begin{figure}[ht]
    \lstinputlisting[language=Java,caption={BeginScope als Java-Interface},label={LST:JOIBeginScopeJava}]{joi/BeginScope.java}
\end{figure}
\begin{figure}[ht]
    \lstinputlisting[language=Scala,caption={BeginScope als Scala-Trait},label={LST:JOIBeginScopeScala}]{joi/BeginScope.scala}
\end{figure}
...
\subsection{Unterstützung für Scala-spezifische Konstrukte}\label{SEC:ScalaFeatures}

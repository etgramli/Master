...
In diesem Abschnitt wird darauf eingegangen, wie die Rekursion der ENBF-Konstrukte behandelt werden können. Die Fälle von Alternative und Schleifen in einer Sequenz werden aus Trivialität übersprungen.
...
\centering
  \begin{subfigure}[c]{0.49\textwidth}
  	\begin{tikzpicture}
		...
		\path[->,draw,thick, bend left] (d) edge node[below] {NodeEdgeA} (b);
		\path[->,draw,thick] (b) edge node[below] {NodeEdgeB} (c);
		\path[->,draw,thick] (c) edge node[below] {NodeEdgeC} (d);
	\end{tikzpicture}

...
\centering
  \begin{subfigure}[c]{0.49\textwidth}
  	\begin{tikzpicture}
		...
		\path[->,draw,thick, bend left] (a) edge node[below] {NodeEdgeA} (b);
		\path[->,draw,thick, bend left] (b) edge node[left] {NodeEdgeB} (c);
		\path[->,draw,thick, bend left] (c) edge node[below] {NodeEdgeC} (a);
	\end{tikzpicture}
...
\centering
  \begin{subfigure}[c]{0.49\textwidth}
  	\begin{tikzpicture}
		...
		\path[->,draw,thick, bend left] (a) edge node[below] {AlternativeA} (b);
		\path[->,draw,thick] (a) edge node[below] {AlternativeB} (b);
		\path[->,draw,thick, bend left] (b) edge node[below] {NodeEdge} (a);
	\end{tikzpicture}
...
\end{itemize}
Diese Eigenschaften bezeichnen einen gerichteter, zyklischen Multi-/Pseudograph. Die parallelen Kanten haben eine eigene Identität (Name und Typ), mit der man sie von anderen Kanten mit dem selben Quell- und Zielknoten unterscheiden kann.
